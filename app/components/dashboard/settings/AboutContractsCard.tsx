...
const OMIX_TOKEN = "0xb9A091213246Ee02c8BFF4e86D84b0C3536d0631";
    ...
    <div className="flex items-center justify-between gap-3 py-2">
      ...
      {link ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-xs text-lime-300 hover:text-lime-200 underline break-all text-right"
        >
          {value}
        </a>
      ) : (
        <p className="text-xs text-gray-200 break-all text-right">{value}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
      ...
      <div className="rounded-xl border border-white/10 bg-black/30 p-3">
        {row("OMIX Token", OMIX_TOKEN, addrLink(OMIX_TOKEN))}
