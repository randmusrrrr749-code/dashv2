...
const DAILY_OMIX_GROWTH = 0.002; // +0.2% per day (compounded)

// Pick a fixed “simulation start” timestamp (edit to whatever you want)
const SIM_START = new Date("2025-12-29T20:30:00Z").getTime();
const MS_PER_DAY = 24 * 60 * 60 * 1000;

// deterministic "random" in [0, 1) from an integer seed (stable forever)
...
export default function DashboardOverviewPage() {
  ...
  const elapsedMs = Math.max(0, now - SIM_START);
  ...
  const totalOmixSold = useMemo(() => {
    return BASE_TOTAL_OMIX_SOLD * Math.pow(1 + DAILY_OMIX_GROWTH, elapsedDaysFloat);
  }, [elapsedDaysFloat]);

  /**
   * Buyers:
   * - each day has a stable target increment in [10..20]
   * - we "drip" that day's increment smoothly across the day (no daily jumps)
   */
  const totalBuyers = useMemo(() => {
    const wholeDays = Math.floor(elapsedDaysFloat);
    const dayProgress = elapsedDaysFloat - wholeDays; // 0..1 within current day

    let buyers = BASE_TOTAL_BUYERS;

    // Add full increments for completed days
    for (let day = 1; day <= wholeDays; day++) {
      const r = seeded01(day);
      const add = 10 + Math.floor(r * 11); // 10..20 inclusive
      buyers += add;
    }

    // Add partial increment for the current (in-progress) day
    const currentDayIndex = wholeDays + 1;
    const rToday = seeded01(currentDayIndex);
    const addToday = 10 + Math.floor(rToday * 11); // 10..20 inclusive
    buyers += addToday * dayProgress; // drip smoothly

    return Math.floor(buyers);
  }, [elapsedDaysFloat]);
  ...
  }, [isConnected, address, purchasedOmix, totalBuyers]);
  ...
  const earnedFromStaking = useMemo(() => {
    ...
    for (const s of activeStakes) {
      ...
      const secondsElapsed = Math.max(
        0,
        Math.min(snappedLock, nowSec - start)
      );
